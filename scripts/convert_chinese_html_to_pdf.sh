...
#     ./scripts/convert_chinese_html_to_pdf.sh --stick 42
#
...
if [ -n "$STICK" ]; then
    # Single stick
    html_files="${INPUT_DIR}/stick_$(printf '%03d' $STICK)_chinese.html"
    ...
    for i in $(seq $FROM $TO); do
        file="${INPUT_DIR}/stick_$(printf '%03d' $i)_chinese.html"
        if [ -f "$file" ]; then
            html_files+=("$file")
        fi
    done
    ...
    # All Chinese sticks
    html_files=(${INPUT_DIR}/stick_*_chinese.html)
fi

total_files=${#html_files[@]}
if [ $total_files -eq 0 ]; then
    echo "No Chinese HTML files found to convert"
    exit 1
fi

current=0
echo "Converting $total_files Chinese HTML files to PDF..."
echo "Using: Chrome headless mode with font loading delay (10s)"
